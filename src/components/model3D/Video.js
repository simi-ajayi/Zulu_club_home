...
const Video = ({ data, queryParams }) => {
  const [videos, setVideos] = useState([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);
  const [taskId, setTaskId] = useState(null);
  const [loadingVideo, setLoadingVideo] = useState({});  // Track loading state of videos
  ...
  const getVideos = async () => {
    try {
      ...
      const unique3DModelVideos = videoData.filter(video => {
        if (!seenNames.has(video.video_name) && video.video_type === "video") {
  ...
  const handleVideoLoadEnd = (videoId) => {
  ...
  return (
    <div className="mt-10">
      <div className="flex justify-between items-center mb-6">
        ...
        <Link to="/AllVideos" className="text-blue-700">
          View All
        </Link>
      </div>
            ...
            <div key={video.id} className="bg-gray-100 p-3 rounded shadow-md relative">
              ...
              <div className="relative w-full h-40 bg-gray-300 flex items-center justify-center rounded-md mb-2">
                ...
                <video
                  ...
                  muted
                  loop
                  onLoadedData={() => handleVideoLoadEnd(video.id)}
